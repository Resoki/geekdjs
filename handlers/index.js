...
const rest = new REST({ version: "9" }).setToken(
  process.env.DJS_TOKEN || global.token
);
...
const loadEvents = async function (client) {
  ...
  for (const folder of eventFolders) {
    ...
    for (const file of eventFiles) {
      ...
      if (event.name) {
        console.log(
          chalk.bgBlueBright.black(` ✔️ => Event ${file} is being loaded `)
        );
      } else {
        console.log(
      ...
      if (command.name) {
        ...
        console.log(
          chalk.bgBlueBright.black(
            ` ✔️ => Prefix Command ${file} is being loaded `
          )
        );
      } else {
        console.log(
          chalk.bgRedBright.black(
            ` ❌ => Prefix Command ${file} missing a help.name or help.name is not in string `
          )
        );
        continue;
      }
  ...
  for (const folder of commandFolders) {
    ...
    for (const file of commandFiles) {
      ...
      if (command.name) {
        ...
        console.log(
          chalk.bgBlueBright.black(
            ` ✔️ => SlashCommand ${file} is being loaded `
          )
        );
      } else {
        console.log(
